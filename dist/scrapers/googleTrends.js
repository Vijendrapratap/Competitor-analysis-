...
import { sleep, toErrorMessage, formatDuration, chunk, round, percentageChange, } from '../utils/helpers.js';
...
export class GoogleTrendsFetcher {
    ...
    async fetchAllTrends() {
        ...
        try {
            log.info(`Fetching Google Trends data for ${this.keywords.length} keywords (geo: ${this.geo}, ${this.timeframeDays}d)`);
            const allTrends = await this.fetchTrends(this.keywords);
            log.info(`Google Trends fetch complete: ${allTrends.length} data points in ${formatDuration(Date.now() - start)}`);
            return {
                success: true,
                data: allTrends,
                error: null,
    ...
    async fetchTrends(keywords) {
        ...
        for (let batchIdx = 0; batchIdx < batches.length; batchIdx++) {
            ...
            log.info(`  Batch ${batchIdx + 1}/${batches.length}: [${batch.join(', ')}]`);
    ...
    async fetchRelatedQueries(keyword) {
        ...
        try {
            const rawJson = await this.callWithRetry(() => googleTrends.relatedQueries({
                keyword,
                startTime,
                endTime,
                geo: this.geo,
            ...
            log.warn(`  Failed to fetch related queries for "${keyword}": ${toErrorMessage(err)}`);
            return [];
        }
    }
    // ─── Batch fetch with retry ─────────────────────────────────────────────
    async fetchBatchWithRetry(keywords) {
        const endTime = new Date();
        const startTime = new Date();
        startTime.setDate(startTime.getDate() - this.timeframeDays);
        const rawJson = await this.callWithRetry(() => googleTrends.interestOverTime({
            keyword: keywords,
            startTime,
            endTime,
            geo: this.geo,
    ...
    processTimeline(timeline, keywords) {
        const now = new Date();
        ...
        for (let kwIdx = 0; kwIdx < keywords.length; kwIdx++) {
            ...
            const avgValue = series.length > 0
                ? round(series.reduce((sum, p) => sum + p.value, 0) / series.length, 2)
                : 0;
            // ── Peak value and date ────────────────────────────────────────────
            let peakValue = 0;
            let peakDate = null;
            ...
            log.info(`  "${keyword}": current=${currentValue}, avg=${avgValue}, peak=${peakValue}, ` +
    ...
    calculateChangePct(series, days) {
        if (series.length < 2)
            return null;
        const latest = series[series.length - 1];
        ...
        for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                if (attempt < this.maxRetries && isRetryable) {
                    ...
                    log.warn(`  API call failed (attempt ${attempt + 1}/${this.maxRetries + 1}): ${msg}. Retrying in ${backoff / 1_000}s`);
                    await sleep(backoff);
                }
                else if (!isRetryable) {
...
//# sourceMappingURL=googleTrends.js.map