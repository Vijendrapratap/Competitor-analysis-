...
import { sleep, sleepRandom, toErrorMessage, formatDuration, normaliseWhitespace, truncate, } from '../utils/helpers.js';
import { settings } from '../config/settings.js';
import { PostType, ContentCategory, } from '../types/index.js';
...
const CATEGORY_KEYWORDS = [
    {
        ...
        keywords: /\b(?:room|suite|villa|pool\s*villa|bungalow|penthouse|accommodation|bedroom|bed\s*type|sea\s*view|ocean\s*view|balcony|ห้อง|ห้องพัก|วิลล่า|สวีท|พูลวิลล่า)\b/i,
    },
    {
        // F&B / Dining
        category: ContentCategory.Brand,
        keywords: /\b(?:restaurant|dining|breakfast|brunch|dinner|lunch|menu|chef|cuisine|cocktail|bar|wine|buffet|food|gastronomy|อาหาร|ร้านอาหาร|บุฟเฟ่ต์|ค็อกเทล|เมนู|อาหารเช้า)\b/i,
    },
    {
        // Wedding
        category: ContentCategory.Event,
        keywords: /\b(?:wedding|bride|groom|ceremony|reception|honeymoon|nuptials|married|engagement|งานแต่ง|แต่งงาน|เจ้าสาว|ฮันนีมูน|พิธี)\b/i,
    },
    {
        // Family
        category: ContentCategory.Educational,
        keywords: /\b(?:family|kids|children|child|playground|kid-friendly|family-friendly|ครอบครัว|เด็ก|สนามเด็กเล่น|กิจกรรมครอบครัว)\b/i,
    },
    {
        // Wellness / Spa
        category: ContentCategory.Testimonial,
        keywords: /\b(?:spa|wellness|massage|yoga|meditation|retreat|relaxation|therapy|healing|detox|สปา|นวด|โยคะ|สุขภาพ|ผ่อนคลาย|รีทรีท)\b/i,
    },
    {
        // Events / MICE
        category: ContentCategory.Event,
        keywords: /\b(?:conference|meeting|event|seminar|workshop|corporate|team\s*building|incentive|banquet|convention|ประชุม|สัมมนา|อีเวนท์)\b/i,
    },
    {
        // Behind the Scenes
        category: ContentCategory.UserGenerated,
        keywords: /\b(?:behind\s*the\s*scenes|bts|team|staff|our\s*people|meet\s*our|เบื้องหลัง|ทีมงาน|พนักงาน)\b/i,
    },
    {
        // Promotion / Offer
        category: ContentCategory.Promotion,
        keywords: /\b(?:promo(?:tion)?|offer|deal|discount|sale|save|free|complimentary|package|special\s*rate|early\s*bird|flash\s*sale|โปรโมชั่น|ส่วนลด|ราคาพิเศษ|ฟรี|แพ็คเกจ|ลดราคา)\b/i,
    },
    {
        // Seasonal / Holiday
        category: ContentCategory.Seasonal,
        keywords: /\b(?:christmas|new\s*year|songkran|loy\s*krathong|chinese\s*new\s*year|valentine|halloween|easter|summer|winter|festive|holiday|season|คริสต์มาส|ปีใหม่|สงกรานต์|ลอยกระทง|ตรุษจีน|วาเลนไทน์)\b/i,
    },
];
// ─────────────────────────────────────────────────────────────────────────────
...
function parseEngagementNumber(text) {
    ...
    if (suffixMatch?.[1] && suffixMatch[2]) {
        ...
        return Math.round(num * (multipliers[suffixMatch[2].toLowerCase()] ?? 1));
    }
    // Thai shorthand
...
export class FacebookPostsScraper {
    ...
    async scrapeAll(competitors, days = 30) {
        if (!this.browser) {
            throw new Error('Browser not initialised — call init() first');
        }
        const results = new Map();
        log.info(`Starting posts scrape for ${competitors.length} competitors (last ${days} days)`);
        for (let i = 0; i < competitors.length; i++) {
            const competitor = competitors[i];
            log.info(`[${i + 1}/${competitors.length}] Scraping posts: ${competitor.name}...`);
            ...
            if (result.success) {
                ...
                log.info(`  \u2714 ${competitor.name}: ${posts.length} posts (${topCount} top performers) in ${formatDuration(result.durationMs)}`);
            }
            else {
        ...
        const totalPosts = [...results.values()].reduce((sum, r) => sum + (r.data?.length ?? 0), 0);
        log.info(`Posts scrape complete: ${successCount}/${competitors.length} succeeded, ${totalPosts} total posts`);
        return results;
    }
    // ─── Scrape single competitor posts (public) ──────────────────────────────
    async scrapePosts(competitor, days = 30) {
        if (!this.browser) {
            throw new Error('Browser not initialised — call init() first');
        }
    ...
    async scrapePostsSafe(competitor, days) {
    ...
    // ─── Core implementation ──────────────────────────────────────────────────
    async scrapePostsImpl(competitor, days) {
        if (!this.browser)
    ...
    async scrollToLoadPosts(page, cutoffDate) {
        let stableRounds = 0;
        let previousCount = 0;
        ...
        for (let round = 0; round < maxScrollRounds; round++) {
            ...
            if (currentCount === previousCount && currentCount > 0) {
                ...
                if (stableRounds >= maxStableRounds) {
                    log.debug(`  No new posts after ${maxStableRounds} scrolls (total: ${currentCount})`);
                    break;
                }
            }
            else {
            ...
            if (currentCount > 0) {
                ...
                if (reachedCutoff) {
                    log.debug(`  Reached date cutoff (${cutoffDate.toISOString().slice(0, 10)}) after ${currentCount} posts`);
                    break;
                }
            }
            // Scroll down
     ...
     */
    async hasReachedDateCutoff(page, cutoffDate) {
        try {
            ...
            const ariaLabel = (await lastTimestamp.getAttribute('aria-label').catch(() => null)) ?? '';
            const title = (await lastTimestamp.getAttribute('title').catch(() => null)) ?? '';
    ...
    async extractAllPosts(page, competitor, cutoffDate) {
        const posts = [];
        const now = new Date();
        ...
        for (let i = 0; i < postCount; i++) {
            try {
                ...
                const post = await this.extractSinglePost(page, postEl, competitor, i, now);
                if (!post)
                    continue;
                // Skip if post is older than cutoff
                if (post.postedAt && post.postedAt < cutoffDate) {
                    log.debug(`  Skipping post from ${post.postedAt.toISOString().slice(0, 10)} (before cutoff)`);
                    continue;
                }
                ...
                log.warn(`  Failed to extract post #${i + 1} for ${competitor.name}: ${toErrorMessage(err)}`);
            }
        }
        return posts;
    }
    // ─── Extract a single post ────────────────────────────────────────────────
    async extractSinglePost(_page, postEl, competitor, index, now) {
        const fullText = await postEl.innerText().catch(() => '');
        if (!fullText.trim())
            return null;
        // ── Post ID & URL ───────────────────────────────────────────────────────
        const { postId, postUrl } = await this.extractPostIdAndUrl(postEl, competitor, index);
        // ── Timestamp ────────────────────────────────────────────────────────────
    ...
    // ─── Field extractors ─────────────────────────────────────────────────────
    async extractPostIdAndUrl(postEl, competitor, index) {
        try {
            // Look for links that contain post/photo/video/reel IDs
            const links = postEl.locator('a[href*="/posts/"], a[href*="/photos/"], a[href*="/videos/"], a[href*="/reel/"], a[href*="story_fbid"]');
            ...
            for (let i = 0; i < linkCount; i++) {
                ...
                const postIdMatch = /\/posts\/(\w+)|\/photos\/[^/]+\/(\d+)|\/videos\/(\d+)|\/reel\/(\d+)|story_fbid=(\d+)/i.exec(href);
                if (postIdMatch) {
    ...
    async detectPostType(postEl, postUrl) {
        try {
            // Check URL first
            if (postUrl.includes('/reel/'))
                return PostType.Reel;
            if (postUrl.includes('/videos/'))
                return PostType.Video;
            if (postUrl.includes('/photos/'))
                return PostType.Photo;
            if (postUrl.includes('/events/'))
                return PostType.Event;
            // Check for video elements
            const hasVideo = (await postEl.locator('video').count()) > 0 ||
            ...
            const hasExternalLink = (await postEl.locator('a[href*="l.facebook.com"]').count()) > 0;
            if (hasExternalLink)
                return PostType.Link;
            // Check for images
            const hasImage = (await postEl.locator('img[src*="scontent"]').count()) > 0 ||
    ...
    async extractReactions(postEl, fullText) {
        try {
    ...
    async extractVideoViews(postEl, fullText, postType) {
     ...
     */
    scoreAndFlagTopPerformers(posts) {
        if (posts.length === 0)
            return posts;
    ...
    parseRelativeOrAbsoluteDate(text) {
        ...
        const monthFirst = /(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w*\s+(\d{1,2})/i.exec(text);
        if (monthFirst) {
