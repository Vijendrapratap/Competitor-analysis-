 ...
 * ```
 */
export async function scrapeCompetitorFacebookPosts(competitors) {
    log.info(`Starting Facebook posts scrape for ${competitors.length} competitor(s)`);
    const promises = competitors.map((competitor) => scrapeOneCompetitor(competitor));
 ...
 * ```
 */
export function filterRecentPosts(posts, daysBack = 7) {
...
async function scrapeOneCompetitor(competitor) {
...
function normalizePost(raw, competitorName) {
    return {
        competitorName,
...
function inferPostType(raw) {
    ...
    if (raw.videoUrl)
        return 'video';
    if (raw.photoUrl || (raw.images && raw.images.length > 0))
        return 'photo';
    if (raw.postUrl?.includes('permalink'))
        return 'link';
...
//# sourceMappingURL=facebookPostsScraper.js.map