 ...
 */
export async function withRetry(fn, maxRetries = 3, delayMs = 1_000, label = 'operation') {
    let lastError;
...
export function parseNumber(raw) {
    ...
    const suffixMap = { k: 1_000, m: 1_000_000, b: 1_000_000_000 };
    const suffixMatch = /^([\d.]+)\s*([kmb])$/i.exec(s);
    if (suffixMatch) {
        const [, num, suffix] = suffixMatch;
        const multiplier = suffixMap[suffix.toLowerCase()] ?? 1;
 ...
 */
export function formatDate(date, locale = 'en-GB', opts = { day: '2-digit', month: 'short', year: 'numeric' }) {
    if (date == null)
        return '—';
 ...
 */
export function truncate(text, maxLen, suffix = '…') {
