    ...
    for (const attemptModel of modelsToTry) {
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                log.info(`LLM OK  model=${attemptModel}  tokens=${usage.totalTokens}  elapsed=${elapsed.toFixed(1)}s`);
                const result = {
                    content,
                    model: attemptModel,
                    usage,
                ...
                sessionStats.errors++;
                log.warn(`LLM ERROR  model=${attemptModel}  attempt=${attempt}/${MAX_RETRIES}  error=${lastError.message.substring(0, 200)}`);
...
`;
export async function analyzeCompetitorStrategy(competitorData) {
    ...
    try {
        ...
        const retry = await callLlm(COMPETITOR_SYSTEM_PROMPT, userPrompt + '\n\nREMINDER: Return ONLY the JSON object, nothing else.', { useCache: false });
...
{{competitors_block}}
`;
export async function generateMarketIntelligence(allCompetitors) {
    ...
    try {
        ...
        const retry = await callLlm(MARKET_SYSTEM_PROMPT, userPrompt + '\n\nREMINDER: Return ONLY the JSON object.', { maxTokens: 3000, useCache: false });
...
// ===========================================================================
export function generateExecutiveBriefText(marketIntel) {
    const lines = [];
...
export function getSessionCostSummary() {
    ...
    stats.estimated_cost_usd = Number((inputCost + outputCost).toFixed(4));
    return stats;
}
