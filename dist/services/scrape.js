...
import { scrapeCompetitorFacebookPosts, } from '../scrapers/facebookPostsScraper.js';
import { getCompetitors, getCompetitorById, insertAds, insertPageMetrics, insertPost, insertTrend, insertFollowerHistory, markAdsInactive, } from '../db/queries.js';
import { CreativeType, CtaType, Platform, PostType, ContentCategory, } from '../types/index.js';
...
function mapApifyAdToNewAd(raw, competitor) {
    return {
        ...
        creativeType: mapCreativeType(raw['mediaType']),
        adCopy: raw['adText'] || null,
        headline: raw['adHeadline'] || null,
        ctaType: mapCtaType(raw['callToAction']),
...
function mapApifyPostToNewPost(post, competitor) {
    return {
        competitorId: competitor.id,
...
function mapPlatforms(raw) {
    ...
    return raw.map((p) => {
        ...
        if (lower.includes('messenger'))
            return Platform.MessengerInbox;
        if (lower.includes('audience'))
            return Platform.AudienceNetwork;
        return Platform.Facebook;
    });
}
function mapCreativeType(raw) {
    if (!raw)
 ...
 */
function extractPartialPageMetrics(postsResult, competitor) {
...
export class ScrapeService {
    ...
    async run(opts = {}) {
        ...
        if (competitorIds && competitorIds.length > 0) {
            ...
            competitors = results.filter((c) => c !== null);
        ...
        const postsResultsByCompetitor = new Map();
        ...
        try {
            ...
            for (const adsResult of adsResults) {
                ...
                const ads = adsResult.ads.map((raw) => mapApifyAdToNewAd(raw, competitor));
        ...
        try {
            ...
            for (const postsResult of postsResults) {
                ...
                const posts = postsResult.posts.map((raw) => mapApifyPostToNewPost(raw, competitor));
                if (!dryRun) {
                ...
                if (partialMetrics) {
                    log.info(`  Partial page data from posts: ${partialMetrics.postsLast30d} posts, ` +
