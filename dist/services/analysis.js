...
import { getCompetitors, getCompetitorById, getCustomer, getCompetitorAds, getLatestPageMetrics, getRecentPosts, getLatestAnalysis, getRecentTrends, getShareOfVoice, insertAnalysis, insertAlert, } from '../db/queries.js';
...
export class AnalysisService {
    ...
    async run(opts = {}) {
        ...
        try {
            ...
            const scores = calculator.calculateAll(competitorData.map((d) => ({
        ...
        try {
            ...
            const strategies = await classifier.analyzeAll(competitorData.map((d) => ({
                competitor: d.competitor,
                ads: d.ads,
                posts: d.posts,
                pageMetrics: d.pageMetrics,
            ...
            if (!dryRun) {
                ...
                for (const cd of competitorData) {
                    ...
                    const adTypes = [...typeCounts.entries()].map(([type, count]) => ({
                        type: type,
                        count,
        ...
        try {
            ...
            const currentSnapshots = competitorData.map((d) => ({
                competitor: d.competitor,
                ads: d.ads,
                posts: d.posts,
                pageMetrics: d.pageMetrics,
            ...
            const trendSnapshot = trendData.length > 0 ? { trends: trendData } : undefined;
            ...
            if (customer) {
                ...
                if (customerEntry) {
                    ...
                    const healthScores = competitorData
                        .map((d) => d.healthScore)
                        .filter((hs) => hs !== null);
                    const leader = healthScores.reduce((best, hs) => (!best || hs.totalScore > best.totalScore ? hs : best), null);
                    const totalAds = competitorData.reduce((sum, d) => sum + d.ads.length, 0);
                    const totalFollowers = competitorData.reduce((sum, d) => sum + (d.pageMetrics?.followers ?? 0), 0);
                    const totalEngagement = competitorData.reduce((sum, d) => sum + (d.pageMetrics?.avgEngagementRate ?? 0), 0);
