...
import { round, toErrorMessage, truncate, sleep, } from '../utils/helpers.js';
...
const PROMO_KEYWORDS = /\b(?:promo|offer|deal|package|rate|price|book|reserve|special|sale|flash|early\s*bird|ราคา|จอง|แพ็คเกจ|โปรโมชั่น|ส่วนลด|พิเศษ)\b/i;
// ─────────────────────────────────────────────────────────────────────────────
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
function toNum(raw) {
...
function isReasonableHotelPrice(value, currency) {
    if (currency === 'THB') {
...
export class PricingExtractor {
    ...
    extractFromText(text) {
        ...
        const confidence = uniquePrices.length >= 2
            ? 'high'
    ...
    async extractWithAI(text) {
        ...
        try {
            const message = await this.client.messages.create({
                ...
                system: 'You are a pricing data extraction specialist for Thai hotel ads. ' +
  ...
  "priceContext": "brief note on pricing strategy"
    ...
    async extractAll(ads) {
        ...
        if (allPrices.length === 0 && this.client) {
            const promoAds = ads.filter((a) => a.adCopy && PROMO_KEYWORDS.test(a.adCopy));
            if (promoAds.length > 0) {
                ...
                for (const ad of promoAds.slice(0, 2)) {
                    const aiResult = await this.extractWithAI([ad.adCopy, ad.headline].filter(Boolean).join('\n'));
                    for (const rate of aiResult.roomRates) {
                        const currency = rate.currency?.toUpperCase() === 'USD' ? 'USD' : 'THB';
                    ...
                    for (const pkg of aiResult.packages) {
                        const currency = pkg.currency?.toUpperCase() === 'USD' ? 'USD' : 'THB';
        ...
        // Build discounts
        const discountList = uniqueDiscounts.map((d) => ({
            description: d.raw,
            discountPct: d.pct,
        ...
        const packagePrices = uniquePrices
        ...
        const averageRate = priceValues.length > 0
     ...
     */
    determinePricePosition(avgRate, currency) {
        if (avgRate === null)
            return PricePosition.Average;
        // Convert USD to THB-equivalent for comparison (approximate)
        const thbRate = currency === 'USD' ? avgRate * 35 : avgRate;
        if (thbRate < 2_000)
            return PricePosition.Cheapest;
        if (thbRate < 4_000)
            return PricePosition.BelowAverage;
        if (thbRate < 8_000)
            return PricePosition.Average;
        if (thbRate < 15_000)
            return PricePosition.AboveAverage;
    ...
    deduplicateDiscounts(discounts) {
        const seen = new Set();
