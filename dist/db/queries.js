...
// =============================================================================
import { and, avg, count, desc, eq, gte, notInArray, sql, } from 'drizzle-orm';
import { db } from './index.js';
import { ads, alerts, analyses, competitors, facebookPages, facebookPosts, followerHistory, reports, trends, } from './schema.js';
...
export async function getCompetitorByName(name) {
    const [row] = await db
        .select()
        .from(competitors)
...
export async function insertCompetitor(data) {
...
export async function toggleCompetitorActive(id) {
    const [row] = await db
...
export async function insertAds(dataArray) {
...
export async function getCompetitorAds(competitorId, sinceDate) {
...
export async function getActiveAdsCount(competitorId) {
    const [result] = await db
        .select({ count: count() })
        .from(ads)
        .where(and(eq(ads.competitorId, competitorId), eq(ads.isActive, true)));
    return result?.count ?? 0;
}
export async function getTodayAds() {
    const rows = await db
        .select()
        .from(ads)
...
export async function markAdsInactive(competitorId, activeAdIds) {
    ...
    await db
        .update(ads)
        .set({ isActive: false, updatedAt: new Date() })
        .where(and(eq(ads.competitorId, competitorId), eq(ads.isActive, true), notInArray(ads.metaAdId, activeAdIds)));
...
// FACEBOOK PAGES
// ─────────────────────────────────────────────────────────────────────────────
export async function insertPageMetrics(data) {
    const [result] = await db
...
export async function getLatestPageMetrics(competitorId) {
    const [row] = await db
        .select()
        .from(facebookPages)
...
function mapPageMetricsRow(row) {
    return {
        id: row.id,
        competitorId: row.competitorId,
        pageUrl: row.pageUrl,
...
export async function getRecentPosts(competitorId, days = 30) {
    const since = daysAgoDate(days);
    const rows = await db
        .select()
        .from(facebookPosts)
        .where(and(eq(facebookPosts.competitorId, competitorId), gte(facebookPosts.postedAt, since)))
...
// FOLLOWER HISTORY
// ─────────────────────────────────────────────────────────────────────────────
export async function insertFollowerHistory(competitorId, followers) {
    ...
    const changePct = prev && prev.followers > 0
...
export async function getLatestAnalysis(competitorId) {
    const [row] = await db
        .select()
        .from(analyses)
...
export async function getHistoricalAnalysis(competitorId, days = 90) {
    const since = daysAgoDate(days);
    const rows = await db
        .select()
        .from(analyses)
        .where(and(eq(analyses.competitorId, competitorId), gte(analyses.analysisDate, since)))
        .orderBy(desc(analyses.analysisDate));
    return rows.map(mapAnalysisRow);
}
...
// TRENDS
// ─────────────────────────────────────────────────────────────────────────────
export async function insertTrend(data) {
    const [result] = await db
        .insert(trends)
        .values(data)
        .onConflictDoUpdate({
...
export async function getRecentTrends(days = 30) {
    const since = daysAgoDate(days);
    const rows = await db
        .select()
        .from(trends)
...
export async function markReportDelivered(reportDate, deliveredTo = []) {
    await db
        .update(reports)
        .set({
        status: 'delivered',
...
export async function getMarketOverview() {
    ...
    const [compCounts] = await db
        .select({
        total: count(),
        active: count(sql `CASE WHEN ${competitors.isActive} THEN 1 END`),
    })
        .from(competitors);
    // Total active ads
...
export async function getHealthLeaderboard(analysisDate) {
