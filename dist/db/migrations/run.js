...
async function ensureDatabaseExists() {
    ...
    try {
        ...
        // Check existence
        const result = await adminClient.query(`SELECT EXISTS(SELECT 1 FROM pg_database WHERE datname = $1) AS exists`, [DB_NAME]);
        if (result.rows[0]?.exists) {
            ...
            await adminClient.query(`CREATE DATABASE "${DB_NAME}" ENCODING 'UTF8' LC_COLLATE 'en_US.UTF-8' LC_CTYPE 'en_US.UTF-8' TEMPLATE template0`).catch(async (err) => {
...
// ─────────────────────────────────────────────────────────────────────────────
async function runSchemaPush(pool) {
...
async function seedCompetitors(pool) {
    const db = drizzle(pool, { schema });
    ...
    // Insert in one batch
    await db.insert(competitors).values(SEED_COMPETITORS.map((c) => ({
        name: c.name,
