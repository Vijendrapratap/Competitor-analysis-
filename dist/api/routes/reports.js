...
import { htmlToPdfBuffer, competitorToPdfBuffer, } from '../../services/pdfService.js';
...
// ─────────────────────────────────────────────────────────
reportsRouter.get('/trends/:name', async (req, res, next) => {
    try {
        const trend = await db.getCompetitorTrend(decodeURIComponent(String(req.params['name'])), req.query['months'] ? Number(req.query['months']) : 6);
...
// ─────────────────────────────────────────────────────────
reportsRouter.get('/:uuid', async (req, res, next) => {
    try {
...
// ─────────────────────────────────────────────────────────
reportsRouter.get('/:uuid/pdf', async (req, res, next) => {
    try {
        const report = await db.getReportByUuid(String(req.params['uuid']));
        if (!report) {
            res.status(404).json({ success: false, error: 'Report not found' });
...
// ─────────────────────────────────────────────────────────
reportsRouter.get('/:uuid/view', async (req, res, next) => {
    try {
        const report = await db.getReportByUuid(String(req.params['uuid']));
        if (!report) {
            res.status(404).json({ success: false, error: 'Report not found' });
...
// ─────────────────────────────────────────────────────────
reportsRouter.get('/:uuid/competitors', async (req, res, next) => {
    try {
        const report = await db.getReportByUuid(String(req.params['uuid']));
        if (!report) {
            res.status(404).json({ success: false, error: 'Report not found' });
...
// ─────────────────────────────────────────────────────────
reportsRouter.get('/:uuid/competitors/:name/pdf', async (req, res, next) => {
    try {
        const report = await db.getReportByUuid(String(req.params['uuid']));
        if (!report) {
            res.status(404).json({ success: false, error: 'Report not found' });
        ...
        const comp = competitors.find(c => c.competitorName.toLowerCase() === name.toLowerCase());
        if (!comp?.competitorHtml) {
            res.status(404).json({ success: false, error: 'Competitor not found or no HTML stored' });
            return;
        }
        const pdf = await competitorToPdfBuffer(comp.competitorHtml, `${comp.competitorName} — ${report.title}`);
...
// ─────────────────────────────────────────────────────────
reportsRouter.patch('/:id/restore', async (req, res, next) => {
    try {
        const id = Number(req.params['id']);
...
// ─────────────────────────────────────────────────────────
reportsRouter.patch('/:id/archive', async (req, res, next) => {
    try {
        const id = Number(req.params['id']);
