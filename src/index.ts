...
const competitorIdsOption = new Option(
  '-c, --competitors <ids>',
  'Comma-separated competitor IDs to target (default: all active)',
).argParser((v: string) =>
  v.split(',').map((id) => {
    const n = parseInt(id.trim(), 10);
    if (isNaN(n)) throw new Error(`Invalid competitor ID: "${id}"`);
    return n;
  }),
);
...
const dryRunOption = new Option(
  '--dry-run',
  'Skip side-effects (email delivery, DB writes) and just log what would happen',
).default(false);

const forceOption = new Option(
  '--force',
  'Force execution even if fresh data already exists',
).default(false);
...
function handleCommandError(stage: string, err: unknown): never {
...
async function cmdTestEmail(opts: { to?: string }): Promise<void> {
...
program
  ...
  .action(async (opts: { competitors?: number[]; dryRun: boolean; force: boolean }) => {
    try {
...
program
  ...
  .action(async (opts: { competitors?: number[]; dryRun: boolean; force: boolean; output?: string }) => {
    try {
...
program
  ...
  .option(
    '--recipients <emails>',
    'Override recipient list (comma-separated)',
    (v: string) => v.split(',').map((e) => e.trim()),
  )
  .action(async (opts: { dryRun: boolean; recipients?: string[] }) => {
    try {
