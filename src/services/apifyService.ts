...
/** Shape of an Apify error response body. */
 ...
 */
export function validateApifyToken(): string {
    const token = process.env['APIFY_API_TOKEN'];
    if (!token || token.trim().length === 0) {
        throw new Error(
            '[ApifyService] Missing environment variable APIFY_API_TOKEN. ' +
...
export async function runApifyActor<T = Record<string, unknown>>(
    ...
    if (options.timeout !== undefined) url.searchParams.set('timeout', String(options.timeout));
    if (options.memory !== undefined) url.searchParams.set('memory', String(options.memory));
    ...
    if (options.fields !== undefined) url.searchParams.set('fields', options.fields);
    ...
    try {
        ...
        if (!response.ok) {
            ...
            switch (response.status) {
                case 400: {
                    ...
                    throw new Error(
                        `[ApifyService] Actor "${actorId}" run failed: ${detail}`,
                    );
                }
                case 401:
                    throw new Error(
                        '[ApifyService] Invalid or expired APIFY_API_TOKEN. ' +
                        'Check your token at https://console.apify.com/account/integrations',
                    );
                case 404:
                    throw new Error(
                        `[ApifyService] Actor "${actorId}" not found. ` +
                        'Verify the actor ID is correct and that you have access to it.',
                    );
                case 408:
                    throw new Error(
                        `[ApifyService] Actor "${actorId}" timed out after ${options.timeout ?? DEFAULT_TIMEOUT_SECS}s. ` +
                ...
                default: {
                    ...
                    throw new Error(
                        `[ApifyService] Actor "${actorId}" returned HTTP ${response.status}: ${msg}`,
                    );
                }
            }
        }
        ...
        if (err instanceof DOMException && err.name === 'AbortError') {
            throw new Error(
                `[ApifyService] Actor "${actorId}" request aborted — local timeout of ${timeoutMs / 1000}s exceeded.`,
            );
        }
 ...
 * ```
 */
export async function scrapeFacebookPosts<T = Record<string, unknown>>(
    inputPayload: Record<string, unknown>,
    options: ApifyRunOptions = {},
): Promise<T[]> {
    return runApifyActor<T>('apify~facebook-posts-scraper', inputPayload, options);
}
 ...
 * ```
 */
export async function scrapeMetaAds<T = Record<string, unknown>>(
    inputPayload: Record<string, unknown>,
    options: ApifyRunOptions = {},
): Promise<T[]> {
    return runApifyActor<T>('leadsbrary~meta-ads-library-scraper', inputPayload, options);
}
...
async function parseErrorBody(response: Response): Promise<ApifyErrorBody | null> {
    try {
        return (await response.json()) as ApifyErrorBody;
    } catch {
        return null;
    }
}
