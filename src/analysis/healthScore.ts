...
// ─────────────────────────────────────────────────────────────────────────────
// Input type — everything the calculator needs for one competitor
// ─────────────────────────────────────────────────────────────────────────────
...
export class HealthScoreCalculator {
  private readonly paidWeight: number;
  private readonly organicWeight: number;
  ...
  calculateAll(allData: CompetitorData[]): HealthScore[] {
    ...
    log.info(
      `Calculated health scores for ${scores.length} competitors ` +
  ...
  calculateScore(data: CompetitorData, ctx: MarketContext): HealthScore {
    ...
    const paidRaw =
      adVolume.score + adFreshness.score + adCreativity.score + adPlatformCoverage.score;

    // ── ORGANIC components ─────────────────────────────────────────────────
    const audienceSize = this.scoreFollowerSize(data.pageMetrics?.followers ?? null);
    const engagementRate = this.scoreEngagementRate(
      data.pageMetrics?.avgEngagementRate ?? null,
    );
    const postFrequency = this.scorePostingFrequency(data.posts, data.pageMetrics);
    const contentDiversity = this.scoreContentDiversity(
      data.posts,
      data.pageMetrics?.rating ?? null,
    );
    ...
    const totalScore = round(
      (paidScore * this.paidWeight + organicScore * this.organicWeight) / 100,
      1,
    );
    ...
    log.debug(
      `  ${data.competitor.name}: total=${totalScore} (paid=${paidScore}, organic=${organicScore}) [${trend}]`,
    );
   ...
   */
  private scoreAdVolume(adCount: number, maxAds: number): ScoreComponent {
   ...
   *   no ads    → 0
   */
  private scoreAdFreshness(ads: Ad[], now: Date): ScoreComponent {
    if (ads.length === 0) {
      return {
        score: 0,
        maxScore: 10,
    ...
    const daysSince = Math.floor(
      (now.getTime() - newestDate.getTime()) / (1_000 * 60 * 60 * 24),
    );
   ...
   */
  private scoreAdCreativity(ads: Ad[]): ScoreComponent {
    if (ads.length === 0) {
      return {
        score: 0,
        maxScore: 20,
    ...
    const uniqueTypes = new Set<CreativeType>(ads.map((a) => a.creativeType));
    ...
    return {
      ...
      label: 'Ad Creativity',
      explanation:
        `${uniqueTypes.size} creative type${uniqueTypes.size !== 1 ? 's' : ''} with up to ${maxVariations} variation${maxVariations !== 1 ? 's' : ''} ` +
   ...
   */
  private scoreMarketPosition(shareOfVoice: number): ScoreComponent {
   ...
   *    null → 0
   */
  private scoreFollowerSize(followers: number | null): ScoreComponent {
   ...
   */
  private scoreEngagementRate(rate: number | null): ScoreComponent {
    if (rate === null) {
      return {
        score: 0,
        maxScore: 10,
    ...
    let tier: string;
    if (rate >= 3) tier = 'excellent';
    else if (rate >= 1.5) tier = 'good';
    else if (rate >= 0.5) tier = 'average';
    else tier = 'low';
  ...
  private scorePostingFrequency(
    ...
    if (pageMetrics?.postsLast30d != null && pageMetrics.postsLast30d > 0) {
      ...
      postsLast30d = posts.filter(
        (p) => p.postedAt && p.postedAt >= cutoff,
      ).length;
    }
  ...
  private scoreContentDiversity(
    ...
    let reputationExplanation: string;
    ...
    return {
      ...
      explanation:
        `${qualityExplanation}; ${reputationExplanation} ` +
  ...
  private determineTrend(
    ...
    if (delta > 3) return Trend.Rising;
    if (delta < -3) return Trend.Declining;
    return Trend.Stable;
  }
  ...
  private formatFollowers(count: number): string {
    if (count >= 1_000_000) return `${round(count / 1_000_000, 1)}M`;
    if (count >= 1_000) return `${round(count / 1_000, 1)}K`;
    return String(count);
  }
}
