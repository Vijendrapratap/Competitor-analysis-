 ...
 */
const SEASONAL_THEMES: Array<{ label: string; pattern: RegExp }> = [
...
export class AlertDetector {
  ...
  detectAlerts(
    currentData: CompetitorSnapshot[],
    historicalData: Map<number, CompetitorHistory>,
    ...
    log.info(
      `Detecting alerts for ${currentData.length} competitors...`,
    );
    ...
    for (const current of currentData) {
      ...
      try {
        const competitorAlerts = this.detectCompetitorAlerts(
          current,
          history ?? null,
          now,
        );
        ...
        log.warn(
          `  Alert detection failed for ${current.competitor.name}: ${toErrorMessage(err)}`,
        );
      }
    }
    ...
    // ── Trend checks ──────────────────────────────────────────────────
    if (trendSnapshot) {
      alerts.push(...this.detectTrendSpikes(trendSnapshot, now, currentData));
    }

    log.info(
      `Detected ${alerts.length} alert${alerts.length !== 1 ? 's' : ''}: ` +
  ...
  private detectCompetitorAlerts(
    ...
    history: CompetitorHistory | null,
    now: Date,
  ): NewAlert[] {
    const alerts: NewAlert[] = [];
    ...
    if (!history && ads.length > 0) {
      alerts.push(
        this.makeAlert(now, AlertType.NewCompetitor, AlertSeverity.Critical, cid, competitor.name,
          `New advertiser detected: ${competitor.name}`,
          `${competitor.name} has appeared in the ad landscape with ${ads.length} active ad${ads.length !== 1 ? 's' : ''}. ` +
          `No historical data exists, indicating this is a new entrant or a competitor that has resumed advertising.`,
          `Review their ad creatives and landing pages to understand their positioning. ` +
    ...
    if (history && history.previousAds.length >= 5 && ads.length === 0) {
      ...
      if (daysSinceLastAd >= 30) {
        alerts.push(
          this.makeAlert(now, AlertType.NewCampaign, AlertSeverity.Critical, cid, competitor.name,
            `Competitor went dark: ${competitor.name}`,
            `${competitor.name} had ${history.previousAds.length} active ads but now has zero. ` +
            `Their newest known ad is ${daysSinceLastAd} days old. This may indicate a strategic pause, ` +
            `budget reallocation, or preparation for a major new campaign.`,
            `Monitor for reactivation. Consider increasing your own ad presence to capture their share of voice.`,
          ),
        );
      }
    }
    ...
    if (history) {
      ...
      if (prevMaxVariations <= 2 && currMaxVariations >= 5) {
        alerts.push(
          this.makeAlert(now, AlertType.NewCampaign, AlertSeverity.Warning, cid, competitor.name,
            `Campaign scaling: ${competitor.name}`,
            `${competitor.name} scaled their ad variations from ${prevMaxVariations} to ${currMaxVariations}. ` +
            `This indicates A/B testing at scale and a likely budget increase.`,
            `Analyse their new creatives for messaging shifts. Consider diversifying your own ad variations.`,
          ),
        );
      }
    }
    ...
    if (history && history.previousAds.length > 0) {
      ...
      if (growth >= 50 && ads.length >= 5) {
        alerts.push(
          this.makeAlert(now, AlertType.HighAdVolume, AlertSeverity.Warning, cid, competitor.name,
            `Ad surge: ${competitor.name} (+${round(growth, 0)}%)`,
            `${competitor.name} increased from ${history.previousAds.length} to ${ads.length} active ads ` +
            `(+${round(growth, 0)}% week over week). This aggressive expansion suggests a new campaign launch or budget spike.`,
            `Review the new ads for themes or offers that may compete directly with your campaigns.`,
          ),
        );
      }
    }
    ...
    // ── HIGH: Rating drop ────────────────────────────────────────────
    if (
      history?.previousPageMetrics?.rating != null &&
      pageMetrics?.rating != null
    ) {
      const drop = history.previousPageMetrics.rating - pageMetrics.rating;
      if (drop >= 0.3) {
        alerts.push(
          this.makeAlert(now, AlertType.ThreatEscalation, AlertSeverity.Warning, cid, competitor.name,
            `Rating drop: ${competitor.name} (${history.previousPageMetrics.rating} → ${pageMetrics.rating})`,
            `${competitor.name}'s page rating dropped from ${history.previousPageMetrics.rating} to ${pageMetrics.rating} ` +
            `(−${round(drop, 1)} stars). This may indicate service quality issues or negative press.`,
            `Monitor their reviews for specific complaints. This could be an opportunity to differentiate on quality.`,
          ),
        );
      }
    }
    ...
    if (history && history.avgEngagement > 0) {
      ...
      for (const vp of viralPosts.slice(0, 2)) {
        ...
        alerts.push(
          this.makeAlert(now, AlertType.ViralContent, AlertSeverity.Warning, cid, competitor.name,
            `Viral content: ${competitor.name}`,
            `A ${vp.postType} post from ${competitor.name} received ${vp.reactions} reactions, ` +
            ...
            `Category: ${vp.contentCategory}.`,
            `Study the content format and topic. Consider creating similar content or a timely response.`,
          ),
        );
      }
    }
    ...
    if (history && history.previousAds.length >= 3 && ads.length >= 3) {
      const prevVideoRatio =
        history.previousAds.filter((a) => a.creativeType === 'video').length /
        history.previousAds.length;
      const currVideoRatio =
        ads.filter((a) => a.creativeType === 'video').length / ads.length;

      if (prevVideoRatio < 0.3 && currVideoRatio >= 0.5) {
        alerts.push(
          this.makeAlert(now, AlertType.NewCampaign, AlertSeverity.Warning, cid, competitor.name,
            `Format shift to video: ${competitor.name}`,
            `${competitor.name} shifted from ${round(prevVideoRatio * 100, 0)}% to ${round(currVideoRatio * 100, 0)}% ` +
            `video content in their ads. This suggests a strategic move towards richer media formats.`,
            `Evaluate your own video content strategy. Video typically drives higher engagement in hospitality.`,
          ),
        );
      }
    }
    ...
    if (history && history.previousAds.length > 0 && ads.length > 0) {
      ...
      if (changePct > 10 && changePct < 50 && change >= 2) {
        ...
        alerts.push(
          this.makeAlert(now, AlertType.NewCampaign, AlertSeverity.Info, cid, competitor.name,
            `Ad count ${direction}: ${competitor.name}`,
            `${competitor.name} ${direction} their active ads from ${history.previousAds.length} to ${ads.length} ` +
            `(${direction === 'increased' ? '+' : '−'}${round(changePct, 0)}%).`,
            null,
          ),
        );
      }
    ...
    if (pageMetrics?.followers && history?.previousPageMetrics?.followers) {
      ...
      for (const milestone of milestones) {
        if (
          history.previousPageMetrics.followers < milestone &&
          pageMetrics.followers >= milestone
        ) {
          alerts.push(
            this.makeAlert(now, AlertType.PageMilestone, AlertSeverity.Warning, cid, competitor.name,
              `Milestone: ${competitor.name} reached ${this.formatNumber(milestone)} followers`,
              `${competitor.name} crossed the ${this.formatNumber(milestone)}-follower mark ` +
              `(now at ${this.formatNumber(pageMetrics.followers)}). This enhances their organic reach significantly.`,
              `Review their growth tactics and engagement strategy around this milestone.`,
            ),
          );
  ...
  private detectPriceWar(
    ...
    const heavyDiscounters: Array<{ name: string; maxDiscount: number }> = [];
    ...
    if (heavyDiscounters.length >= 3) {
      ...
      const highest = heavyDiscounters.reduce((max, d) => (d.maxDiscount > max.maxDiscount ? d : max), heavyDiscounters[0]!);
      const compId = currentData.find((c) => c.competitor.name === highest.name)?.competitor.id || currentData[0]?.competitor.id || 0;

      return [
        this.makeAlert(now, AlertType.PriceChange, AlertSeverity.Critical, compId, undefined,
          `Price war detected: ${heavyDiscounters.length} competitors offering 30%+ discounts`,
          `Multiple competitors are running aggressive discount campaigns: ${names}. ` +
          `This indicates a price war in the market that could erode margins across the segment.`,
          `Evaluate whether to match discounts or differentiate on value-add. ` +
  ...
  private detectSeasonalPush(
    ...
    for (const theme of SEASONAL_THEMES) {
      ...
      if (matchingCompetitors.length >= 3) {
        const compId = currentData.find(c => c.competitor.name === matchingCompetitors[0])?.competitor.id || currentData[0]?.competitor.id || 0;
        alerts.push(
          this.makeAlert(now, AlertType.EngagementSpike, AlertSeverity.Warning, compId, undefined,
            `Seasonal push: ${theme.label} (${matchingCompetitors.length} competitors)`,
            `${matchingCompetitors.length} competitors are running ${theme.label}-themed campaigns: ` +
            `${matchingCompetitors.slice(0, 5).join(', ')}${matchingCompetitors.length > 5 ? '...' : ''}. ` +
            `This is a coordinated seasonal push in the market.`,
            `If you haven't launched a ${theme.label} campaign, consider a timely response. ` +
  ...
  private detectTrendSpikes(
    ...
    const alerts: NewAlert[] = [];
    const compId = currentData.find(c => c.competitor.isCustomer)?.competitor.id || currentData[0]?.competitor.id || 0;

    for (const trend of trendSnapshot.trends) {
      if (trend.changePct !== null && trend.changePct >= 50) {
        alerts.push(
          this.makeAlert(now, AlertType.EngagementSpike, AlertSeverity.Warning, compId, undefined,
            `Trend spike: "${trend.keyword}" (+${round(trend.changePct, 0)}%)`,
            `Google Trends data shows "${trend.keyword}" surged ${round(trend.changePct, 0)}% month-over-month ` +
            `(current value: ${trend.value}/100). This indicates rising consumer interest.`,
            `Consider creating content or ads targeting "${trend.keyword}" to capture the demand spike.`,
          ),
        );
      }
    }
  ...
  private daysSinceNewestAd(ads: Ad[], now: Date): number {
