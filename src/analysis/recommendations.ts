...
export interface MarketData {
  ...
  analyses: Array<{ competitor: Competitor; analysis: Analysis }>;
  /** All health scores. */
...
/** Shape we expect from Claude's JSON response. */
interface ClaudeRecommendation {
  priority: 'urgent' | 'important' | 'consider';
  title: string;
  description: string;
  actionItems: string[];
...
export class RecommendationEngine {
  ...
  async generateRecommendations(
    ...
    try {
      ...
      log.info(
        `Generated ${recommendations.length} recommendations in ${formatDuration(Date.now() - start)}: ` +
...
Return ONLY a valid JSON array (no markdown, no explanation).`;
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        const message = await this.client.messages.create({
          ...
          system:
            'You are an expert hotel marketing strategist specialising in the Thai hospitality market. ' +
  ...
  private parseResponse(
    ...
    const validIds = new Set(
      marketData.analyses.map((a) => a.competitor.id),
    );

    return parsed
      .filter(
        (r): r is ClaudeRecommendation =>
          typeof r === 'object' &&
          r !== null &&
      ...
      .map((r) => ({
        ...
        actionItems: Array.isArray(r.actionItems)
          ? r.actionItems.filter((i): i is string => typeof i === 'string').slice(0, 5)
          : [],
          ...
          ? r.relatedCompetitors
            .filter(
              (c): c is { id: number; name: string } =>
                ...
                validIds.has((c as { id: number }).id),
            )
            .slice(0, 5)
          : [],
      }));
  ...
  private generateFallbackRecommendations(
    ...
    if (analysis && analysis.totalActiveAds < marketData.avgAdCount * 0.5) {
      ...
      recs.push({
        ...
        description:
          `You have ${analysis.totalActiveAds} active ads vs. the market average of ${round(marketData.avgAdCount, 0)}. ` +
    ...
    if (pageMetrics?.rating != null && pageMetrics.rating < 4.0) {
      recs.push({
        ...
        description:
          `Your current rating of ${pageMetrics.rating}/5 is below the typical threshold ` +
    ...
    const priceWarAlerts = marketData.recentAlerts.filter(
      (a) => a.alertType === 'price_change' && a.severity === 'critical',
    );
    if (priceWarAlerts.length > 0) {
      recs.push({
        ...
        description:
          `Multiple competitors are running aggressive discount campaigns (30%+ off). ` +
    ...
    if (
      ...
      recs.push({
        ...
        description:
          `Your engagement rate of ${round(pageMetrics.avgEngagementRate, 2)}% is below the ` +
    ...
    if (topTrend) {
      recs.push({
        ...
        description:
          `Google Trends shows "${topTrend.keyword}" is up ${round(topTrend.changePct!, 0)}% ` +
    ...
    if (underserved.length > 0) {
      recs.push({
        ...
        description:
          `The following segments have fewer than 3 active competitors: ` +
    ...
    if (pageMetrics?.followers != null && pageMetrics.followers < marketData.avgFollowers) {
      recs.push({
        ...
        description:
          `Your follower count of ${this.formatNumber(pageMetrics.followers)} is below the ` +
