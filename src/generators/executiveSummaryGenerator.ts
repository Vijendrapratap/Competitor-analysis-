...
env.addFilter('format', function (val: any, formatString: string) {
...
env.addFilter('format_jinja', function (str: string, ...args: any[]) {
    let i = 0;
    return str.replace(/%([.0-9]*)?[df]/g, (match: string, p1: string) => {
        const arg = args[i++];
...
export function generateStrategyChartSvg(
...
function classifyAdTier(adCount: number): string {
    if (adCount >= 10) return 'heavy';
    if (adCount >= 2) return 'moderate';
    if (adCount === 1) return 'light';
    return 'none';
}

function extractStrategies(competitors: any[]): Array<{ name: string; count: number }> {
    const counts: Record<string, number> = {};
    for (const comp of competitors) {
        const strategy = comp.marketing_strategy || comp.strategy || '';
        if (!strategy) continue;

        for (const s of (strategy as string).split(',')) {
 ...
 */
export function generateExecutiveSummary(reportData: any): string {
    ...
    const activeAdvertisers = competitors.filter((c: any) => (c.total_active_ads || 0) > 0).length;
    const activePct = totalResorts ? Math.round((activeAdvertisers / totalResorts) * 100) : 0;

    const totalActiveAds = competitors.reduce((sum: number, c: any) => sum + (c.total_active_ads || 0), 0);
    const avgAds = totalResorts ? totalActiveAds / totalResorts : 0;

    // Ad tier counts
    const tierCounts = { heavy: 0, moderate: 0, light: 0, none: 0 };
    for (const c of competitors) {
        const tier = classifyAdTier(c.total_active_ads || 0) as keyof typeof tierCounts;
        tierCounts[tier]++;
    }
    ...
    const barSvg = generateBarChartSvg(
        tierCounts.heavy, tierCounts.moderate,
        tierCounts.light, tierCounts.none,
    );
