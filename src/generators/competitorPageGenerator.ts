...
function _formatCtaBreakdown(cta: Record<string, number> | null | undefined): string {
...
function _formatNewestDate(dateVal: any): [string, string] {
    /**
...
export function generateCompetitorPage(
    competitor: any,
    healthScore: number = 0.0,
    threatLevel: string = '',
    compNumber: number = 1,
    pageNum: number = 1,
    brandName: string = 'UPMY SKILLS',
    dateEn: string = '',
    dateTh: string = ''
): string {
    /**
    ...
    if (!threatLevel) {
        const estimated = _estimateThreat(
            competitor.total_active_ads || 0,
            healthScore,
            competitor.is_new_entrant || false
        );
...
export function generateAllCompetitorPages(
    competitors: any[],
    ...
    competitors.forEach((comp, i) => {
        ...
        const pageHtml = generateCompetitorPage(
            comp,
            healthScores[name] || 0.0,
            threatLevels[name] || '',
            i + 1,
            startPage + i,
            brandName,
