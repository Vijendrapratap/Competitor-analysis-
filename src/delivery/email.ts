...
export interface ReportSummary {
  ...
  healthScoreLeader: { name: string; score: number } | null;
...
export class EmailDelivery {
  ...
  private config: Required<DeliveryConfig>;
  ...
  async sendReport(pdfPath: string, summary: ReportSummary): Promise<boolean> {
  ...
  async sendCriticalAlert(alert: Alert): Promise<boolean> {
  ...
  private buildReportEmailHtml(summary: ReportSummary): string {
    const alertBadge = (severity: string, count: number, color: string) =>
      count > 0
  ...
  private buildReportEmailText(summary: ReportSummary): string {
    return [
  ...
  private buildAlertEmailHtml(alert: Alert): string {
  ...
  private buildAlertEmailText(alert: Alert): string {
    return [
