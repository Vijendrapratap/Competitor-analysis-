...
(Chart.defaults as any).responsive = false;
(Chart.defaults as any).devicePixelRatio = 1;
...
export class ChartGenerator {
  ...
  async shareOfVoicePie(entries: ShareOfVoiceEntry[]): Promise<string> {
    ...
    const bgColors = top.map((e, i) =>
      e.isCustomer ? COLOURS.gold : PALETTE[i % PALETTE.length]!,
    );
  ...
  async healthScoreBars(entries: HealthScoreEntry[]): Promise<string> {
    ...
    const barColors = sorted.map((e) => {
      if (e.isCustomer) return COLOURS.gold;
      if (e.totalScore >= 70) return COLOURS.green;
      if (e.totalScore >= 40) return COLOURS.navyLight;
      return COLOURS.red;
    });
  ...
  async segmentDistribution(entries: SegmentEntry[]): Promise<string> {
  ...
  async engagementComparison(entries: EngagementEntry[]): Promise<string> {
    ...
    const bgColors = sorted.map((e) =>
      e.isCustomer ? COLOURS.gold : COLOURS.navy,
    );
    const borderColors = sorted.map((e) =>
      e.isCustomer ? COLOURS.goldLight : COLOURS.navyLight,
    );
  ...
  async postingFrequency(entries: PostFrequencyEntry[]): Promise<string> {
    ...
    const bgColors = sorted.map((e) =>
      e.isCustomer ? COLOURS.gold : COLOURS.green,
    );
