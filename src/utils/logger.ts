...
const consoleFormat = winston.format.combine(
  ...
  winston.format.printf(({ timestamp, level, message, context, stack, ...meta }: Record<string, any>) => {
...
export function logStageEnd(stage: string, durationMs: number): void {
  logger.info(`${'─'.repeat(60)}`);
  logger.info(`  STAGE DONE:  ${stage.toUpperCase()}  (${(durationMs / 1_000).toFixed(2)}s)`);
  logger.info(`${'─'.repeat(60)}`);
}

export function logStageError(stage: string, err: unknown): void {
