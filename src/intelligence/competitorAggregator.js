// DEPRECATED — ported to competitorAggregator.ts
// =============================================================================
 ...
 *
 * @param {Array<{ name: string, facebookPageUrl: string, searchTerms?: string[] }>} competitors
 ...
 * @param {Array} allPosts - All fetched posts
 * @param {Array} allAds   - All fetched ads
 * @param {Array} activeAds - Only active ads
 * @param {Array} recentPosts - Posts from last 7 days
 */
function buildSummary(allPosts, allAds, activeAds, recentPosts) {
    ...
    const mostRecentAdDate = adDates.length > 0 ? adDates[0].toISOString() : null;
 ...
 *
 * @param {Array} aggregatedData - Array from `aggregateCompetitorData`
 * @returns {Object} Condensed analysis payload
 */
export function prepareAnalysisPayload(aggregatedData) {
    const competitorPayloads = aggregatedData.map((comp) => {
        ...
        const adCopySamples = comp.activeAds
            .slice(0, 3)
            .map((ad) => ({
 ...
 *
 * @param {Array} aggregatedData - Array from `aggregateCompetitorData`
 * @returns {Object} Comparison table with rows for each competitor
 */
