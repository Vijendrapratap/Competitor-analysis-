...
import {
    ...
    filterRecentPosts,
    CompetitorInput as FacebookPostsCompetitorInput,
    NormalizedPost,
    CompetitorScrapeResult as CompetitorPostsResult
} from '../scrapers/facebookPostsScraper.js';

import {
    scrapeCompetitorMetaAds,
    categorizeAdsByType,
    NormalizedAd,
    CompetitorMetaAdsResult
} from '../scrapers/metaAdsScraper.js';
 ...
 */
export async function aggregateCompetitorData(competitors: AggregatedCompetitorInput[]): Promise<AggregatedCompetitorData[]> {
    ...
    const [postsResults, adsResults] = await Promise.all([
        scrapeCompetitorFacebookPosts(competitors).catch((err) => {
            ...
            return [] as CompetitorPostsResult[];
        }),
        scrapeCompetitorMetaAds(competitors as any).catch((err) => {
            log.error('Meta Ads scraper failed entirely', { error: err.message });
            return [] as CompetitorMetaAdsResult[];
        }),
    ]);

    // ── Index results by competitor name ────────────────────────────────────
    const postsMap = new Map<string, CompetitorPostsResult>();
    for (const r of postsResults) {
        postsMap.set(r.competitorName, r);
    }

    const adsMap = new Map<string, CompetitorMetaAdsResult>();
    ...
    const aggregated = competitors.map((competitor) => {
        ...
        const errors: string[] = [];
        if (postsResult?.error) errors.push(`Posts: ${postsResult.error}`);
        if (adsResult?.error) errors.push(`Ads: ${adsResult.error}`);
 ...
 */
function buildSummary(allPosts: NormalizedPost[], allAds: NormalizedAd[], activeAds: NormalizedAd[], recentPosts: NormalizedPost[]): CompetitorSummary {
 ...
 */
export function prepareAnalysisPayload(aggregatedData: AggregatedCompetitorData[]) {
    ...
    return {
        ...
        analysisPromptHint:
            'Analyze the following competitor intelligence data. ' +
 ...
 */
function extractPostThemes(posts: NormalizedPost[]) {
 ...
 */
export function buildCompetitorComparison(aggregatedData: AggregatedCompetitorData[]) {
    const rows = aggregatedData.map((comp) => {
        ...
        const newCampaigns = comp.activeAds.filter((ad) => {
            if (!ad.adStartDate) return false;
...
function categorizeFrequency(postsPerWeek: number): string {
    if (postsPerWeek >= 7) return 'High (daily+)';
    if (postsPerWeek >= 3) return 'Moderate (3-6/week)';
    if (postsPerWeek >= 1) return 'Low (1-2/week)';
    return 'Inactive';
}

/** Label engagement level as low/moderate/high. */
function categorizeEngagement(avg: number): string {
