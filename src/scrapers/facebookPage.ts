...
export class FacebookPageScraper {
  ...
  async scrapeAll(
  ...
  ): Promise<Map<number, ScrapeResult<NewFacebookPageMetrics>>> {
    if (!this.browser) {
    ...
    try {
      const results = new Map<number, ScrapeResult<NewFacebookPageMetrics>>();
      log.info(`Starting page scrape for ${competitors.length} competitors`);

      for (let i = 0; i < competitors.length; i++) {
        const competitor = competitors[i]!;
        log.info(
          `[${i + 1}/${competitors.length}] Scraping page: ${competitor.name}...`,
        );
        ...
        if (result.success) {
          const d = result.data!;
          log.info(
            `  \u2714 ${competitor.name}: ${d.followers ?? '?'} followers, ${d.pageLikes ?? '?'} likes, rating ${d.rating ?? 'N/A'} (${formatDuration(result.durationMs)})`,
          );
        } else {
          log.error(
      ...
      log.info(
        `Page scrape complete: ${successCount}/${competitors.length} succeeded`,
      );
  ...
  async scrapePage(competitor: Competitor): Promise<NewFacebookPageMetrics> {
    if (!this.browser) {
      throw new Error('Browser not initialised — call init() first');
    }
  ...
  private async scrapePageSafe(
    competitor: Competitor,
  ): Promise<ScrapeResult<NewFacebookPageMetrics>> {
  ...
  private async scrapePageImpl(
    competitor: Competitor,
  ): Promise<NewFacebookPageMetrics> {
    ...
    try {
      ...
      const avgEngagementRate = await this.estimateEngagementRate(
        page,
        followers,
      );
  ...
  private async extractFollowers(page: Page): Promise<number | null> {
    try {
  ...
  private async extractPageLikes(page: Page): Promise<number | null> {
    try {
      const likePatterns = [
  ...
  private async extractRatingAndReviews(
    ...
    try {
      ...
      const fullMatch =
        /([\d.]+)\s*(?:out\s*of\s*5|\/\s*5)?\s*(?:·|based\s*on|from)?\s*([\d,]+)?\s*(?:reviews?|ratings?|รีวิว)?/i.exec(
          mainText,
        );
  ...
  private async extractLastPostDate(page: Page): Promise<Date | null> {
    try {
   ...
   */
  private async countRecentPosts(page: Page): Promise<number | null> {
    try {
      // Scroll a couple of times to load more posts
      for (let i = 0; i < 3; i++) {
        await page.evaluate(() => {
          (window as any).scrollTo({
            top: (document as any).body.scrollHeight,
            behavior: 'smooth',
          });
        });
   ...
   */
  private async estimateEngagementRate(
    page: Page,
   ...
   */
  private parseEngagementFromText(text: string): number {
    let total = 0;
