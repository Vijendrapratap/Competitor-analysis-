...
const CTA_MAP: Record<string, CtaType> = {
...
export class MetaAdsScraper {
  ...
  private readonly maxAdsPerCompetitor: number;
  ...
  async scrapeAll(
  ...
  ): Promise<Map<number, ScrapeResult<NewAd[]>>> {
    if (!this.browser) {
    ...
    try {
      const results = new Map<number, ScrapeResult<NewAd[]>>();
      ...
      for (let i = 0; i < competitors.length; i++) {
        ...
        if (result.success) {
          log.info(
            `  ✔ ${competitor.name}: ${result.data?.length ?? 0} ads in ${formatDuration(result.durationMs)}`,
          );
        } else {
          log.error(
      ...
      const totalAds = [...results.values()].reduce(
        (sum, r) => sum + (r.data?.length ?? 0),
        0,
      );
      log.info(
        `Scrape complete: ${successCount}/${competitors.length} succeeded, ${totalAds} total ads`,
      );
  ...
  async scrapeCompetitor(competitor: Competitor): Promise<NewAd[]> {
    return withRetry(
      () => this.scrapeCompetitorImpl(competitor),
      this.maxRetries,
      5_000,
  ...
  private async scrapeCompetitorSafe(
    competitor: Competitor,
  ): Promise<ScrapeResult<NewAd[]>> {
  ...
  private async scrapeCompetitorImpl(competitor: Competitor): Promise<NewAd[]> {
    ...
    try {
      ...
      const competitorDir = join(
        this.screenshotsDir,
        slugify(competitor.name),
      );
      mkdirSync(competitorDir, { recursive: true });

      // Parse each ad card
      const ads = await this.extractAdsFromPage(
        page,
        competitor,
        competitorDir,
      );
  ...
  private async checkNoAds(page: Page): Promise<boolean> {
    try {
  ...
  private async scrollToLoadAds(page: Page): Promise<number> {
    let previousCount = 0;
    let stableRounds = 0;
    ...
    for (let round = 0; round < 50; round++) {
      ...
      if (bestCount >= this.maxAdsPerCompetitor) {
        log.debug(
          `  Reached ad cap (${this.maxAdsPerCompetitor}), stopping scroll`,
        );
        return bestCount;
      }
      ...
      if (bestCount === previousCount) {
        ...
        if (stableRounds >= maxStableRounds) {
          log.debug(
            `  No new ads after ${maxStableRounds} scrolls (total: ${bestCount})`,
          );
          return bestCount;
        }
      } else {
  ...
  private async extractAdsFromPage(
    page: Page,
    competitor: Competitor,
    ...
    for (let i = 0; i < limit; i++) {
      try {
        ...
        const ad = await this.extractSingleAd(
          page,
          card,
          competitor,
          screenshotDir,
        ...
        log.warn(
          `  Failed to extract ad #${i + 1} for ${competitor.name}: ${toErrorMessage(err)}`,
        );
      }
    }
  ...
  private async extractSingleAd(
    ...
    // ── Screenshot ──────────────────────────────────────────────────────────
    const screenshotPath = await this.takeAdScreenshot(
      card,
      screenshotDir,
      metaAdId,
    );
  ...
  private extractStartedRunning(text: string): Date | null {
    // "Started running on Jun 12, 2024"
    const match =
      /Started\s+running\s+on\s+(\w{3,9}\s+\d{1,2},?\s+\d{4})/i.exec(text);
    if (match?.[1]) {
    ...
    const altMatch =
      /Started\s+running\s+on\s+(\d{1,2}\s+\w{3,9}\s+\d{4})/i.exec(text);
    if (altMatch?.[1]) {
  ...
  private extractPlatforms(text: string): Platform[] {
    const platforms: Platform[] = [];
    const lower = text.toLowerCase();

    if (lower.includes('facebook')) platforms.push(Platform.Facebook);
    if (lower.includes('instagram')) platforms.push(Platform.Instagram);
    if (lower.includes('messenger')) platforms.push(Platform.MessengerInbox);
    ...
    if (platforms.length === 0) platforms.push(Platform.Facebook);

    return platforms;
  }

  private extractVariationsCount(text: string): number {
    // "3 ads use this creative and text"
    const match = /(\d+)\s*ads?\s*use\s*this/i.exec(text);
    return match?.[1] ? parseInt(match[1], 10) : 1;
  }

  private async detectCreativeType(
    card: ReturnType<Page['locator']>,
  ): Promise<CreativeType> {
    try {
      // Check for video elements
      const hasVideo =
        (await card.locator('video').count()) > 0 ||
        (await card.locator('[aria-label*="video" i]').count()) > 0;
      if (hasVideo) return CreativeType.Video;

      // Check for carousel (multiple images or "swipe" indicators)
      const hasCarousel =
        (await card.locator('[aria-label*="carousel" i]').count()) > 0 ||
        (await card.locator('[class*="carousel"]').count()) > 0 ||
        (await card.locator('[aria-label="Next card"]').count()) > 0;
      if (hasCarousel) return CreativeType.Carousel;

      // Check for image
      const hasImage =
        (await card.locator('img[src*="scontent"]').count()) > 0 ||
        (await card.locator('img[src*="fbcdn"]').count()) > 0;
      if (hasImage) return CreativeType.Image;
  ...
  private extractAdCopy(text: string): { adCopy: string | null; headline: string | null } {
    const lines = text
      .split('\n')
    ...
    const contentLines = lines.filter(
      (l) => !metaPatterns.some((p) => p.test(l)),
    );
  ...
  private async extractCta(
    ...
    for (const [key, ctaType] of Object.entries(CTA_MAP)) {
      if (lower.includes(key)) return ctaType;
    }

    return CtaType.Unknown;
  }

  private async extractLandingUrl(
    card: ReturnType<Page['locator']>,
  ): Promise<string | null> {
    try {
  ...
  private async takeAdScreenshot(
    card: ReturnType<Page['locator']>,
    dir: string,
    adId: string,
  ): Promise<string | null> {
    try {
      ...
      log.debug(
        `  Screenshot failed for ad ${adId}: ${toErrorMessage(err)}`,
      );
      return null;
    }
  }
  ...
  private buildAdsLibraryUrl(competitor: Competitor): string {
